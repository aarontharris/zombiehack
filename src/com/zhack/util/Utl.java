...
public final class Utl {
	...
	public static final Box box = new Box(1, 1, 1); // Optimization to reuse the 1x1x1 and just scale the geom. Wont work well with textures. In that
													// case you may cache one of
													// each
													// mesh.
	 ...
	 * @param towardsMe
	 * @param dist
	 *            distance for this frame. 5 will move 5 units. 5/tpf will move 5 units per second.
	 */
	public static void moveTowards(Spatial moveMe, Vector3f towardsMe, float dist) {
		...
		try {
			...
			direction = towardsMe.subtract(moveMe.getLocalTranslation(), direction); // A - B = B's direction towards A with a magnitude of their
																						// distance
	...
	public static final CollisionResult getFirstCollision(CollisionResults collisionResults, Geometry ignore) {
		CollisionResult out = null;
		if (collisionResults != null && collisionResults.size() > 0) {
			int cIdx = 0;
			CollisionResult collision = collisionResults.getCollision(cIdx++);
			Geometry target = collision.getGeometry();
			while (cIdx < collisionResults.size() && target == ignore) {
				collision = collisionResults.getCollision(cIdx++);
