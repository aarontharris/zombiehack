...
public class Pool {
	...
	public static CreepGeometry creepGeometry( String name, float x, float y, float z ) {
		CreepGeometry out = (CreepGeometry) self.nodePool.poll();
		if ( out == null ) {
			self.objectsCreated++;
			out = CreepGeometry.obtainNewCreep( name, x, y, z );
		}
		out.setName( name );
	...
	public static void release( CreepGeometry o ) {
		try {
			self.creepPool.add( o );
		} catch ( Exception e ) {
			e.printStackTrace();
		}
	}
	 ...
	 * @param type
	 * @return
	 * @throws InstantiationException when type does not have a public empty constructor
