...
public class World extends BaseObject {
	...
	public String describePos( int x, int y, int z ) {
		...
		return String.format("World: %s,%s,%s, Sector: %s,%s,%s, Chunk: %s,%s,%s, Block %s,%s,%s",
				x,y,z,
				sector.getX(), sector.getY(), sector.getZ(),
				chunk.getX(), chunk.getY(), chunk.getZ(),
				Chunk.toBlockPos(x, chunk.getX()), Chunk.toBlockPos(y, chunk.getY()), Chunk.toBlockPos( z, chunk.getZ())
				);
	}
}
